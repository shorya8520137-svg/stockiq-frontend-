...
const ProductManager = () => {
    ...
    const fetchProducts = async () => {
        try {
            ...
            if (response.success) {
                ...
                showNotification(response.message || 'Failed to fetch products', 'error');
                setProducts([]);
                setTotalPages(1);
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingProduct) {
                response = await productsAPI.updateProduct(editingProduct.p_id, formData);
            } else {
            ...
            if (response.success) {
                ...
                showNotification(
                    response.message || (editingProduct ? 'Product updated successfully!' : 'Product created successfully!'),
                    'success'
                );
            } else {
    ...
    const handleCategorySubmit = async (e) => {
        ...
        try {
            // For now, just show success message since category API might not be implemented
            showNotification('Category creation not yet implemented (fallback mode)', 'error');
    ...
    const handleScanBarcode = async (barcode) => {
        try {
            showNotification('Barcode scanning not yet implemented (fallback mode)', 'error');
        } catch (error) {
    ...
    const handleExportProducts = async () => {
        try {
            ...
            const response = await productsAPI.getProducts({ limit: 10000 });
