...
echo -e "${YELLOW}📋 Using your SSH key: $SSH_KEY${NC}"
...
if [ $? -eq 0 ]; then
    ...
    echo "scp -i ~/.ssh/ec2-python-ssh.pem controllers/dispatchController.js ubuntu@13.201.222.24:/home/ubuntu/stockiq-frontend-/controllers/"
    exit 1
fi
...
if [ $? -eq 0 ]; then
    ...
    echo -e "${YELLOW}💡 Please run manually:${NC}"
    echo "scp -i ~/.ssh/ec2-python-ssh.pem routes/dispatchRoutes.js ubuntu@13.201.222.24:/home/ubuntu/stockiq-frontend-/routes/"
fi
...
echo "1. Open your dispatch form in the browser"
...
echo ""
echo -e "${YELLOW}🔗 Frontend URL:${NC} https://your-frontend-domain.vercel.app/order/dispatch"
echo -e "${YELLOW}🔗 Backend API:${NC} https://13-201-222-24.nip.io/api/dispatch"
echo ""
echo -e "${GREEN}✅ All dispatch dropdown endpoints should now be working!${NC}"